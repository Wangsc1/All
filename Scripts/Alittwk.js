...
function setSignData() {
 ...
 function createSign(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let url = {
        url : $.getdata('alipayswksignurl'),
        headers : JSON.parse($.getdata('alipayswksignheader')),
        body : $.getdata('alipayswksignbody')
      }
...
function showSignInfo(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      if (typeof $.getdata('alipayswksignurl') === "undefined") {
        $.msg($.name,"",'请先启用获取Cookie脚本然后点击此通知！', openurl)
        return
      }
      let url = {
        url : $.getdata('alipayswksignurl').replace('createSign','showSignInfo'),
        headers : JSON.parse($.getdata('alipayswksignheader'))
      }
      $.post(url, async (err, resp, data) => {
        try {
